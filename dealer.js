...
function renderDeal(){
  ...
  var title=dlrTitle();
  var canSaveDirect=(typeof spToken!=='undefined'&&!!spToken)||document.body.classList.contains('owner');
  ...
  var rowsH=_deal.map(function(e,i){
    ...
    return '<div class="dlr-row">'
      ...
      +'<span class="row-sleeve" data-sleeve="'+t.sid+'"><span class="play-btn" onclick="playPreview(\''+t.sid+'\',this)" title="Play">▶︎</span></span>'
...
/* guests who can't OAuth (dev-mode allowlist) still leave with the selection */
window.copyDealList=function(){
  if(!_deal)return;
  var txt='The Selector: '+dlrTitle()+'\n'
    +_deal.map(function(e,i){
      var line=String(i+1).padStart(2,'0')+'. '+e.t.a.split(';')[0]+' — '+e.t.t;
      return line+((e.t.sid||'').length===22?'\n    https://open.spotify.com/track/'+e.t.sid:'');
    }).join('\n');
  var done=function(){showToast('Tracklist and Spotify links copied')};
  var legacy=function(){
    ...
    ta.remove();ok?done():showToast('Copy blocked — long-press a track to share instead');
  };
